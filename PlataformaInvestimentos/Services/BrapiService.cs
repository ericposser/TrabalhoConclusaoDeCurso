...
public class BrapiService : IBrapi
{
            ...
            if (doc.RootElement.TryGetProperty("coins", out var coins) && coins.ValueKind == JsonValueKind.Array)
            {
                ...
                { 
                    if (!item.TryGetProperty("coin", out var coinProp) ||
                      ...
                      !item.TryGetProperty("coinImageUrl", out var logoProp))
                    {
                        continue; 
                    }
    ...
    public async Task<Brapi?> ObterCripto(string ticker)
    {
        ...
        if (!doc.RootElement.TryGetProperty("coins", out var coins) || coins.ValueKind != JsonValueKind.Array || coins.GetArrayLength() == 0)
            return null;
        ...
        return new Brapi
        {
            ...
            Logo = cripto.GetProperty("coinImageUrl").GetString() ?? string.Empty,
            PrecoAtual = cripto.TryGetProperty("regularMarketPrice", out var preco) && preco.TryGetDecimal(out var precoDec)
                ? precoDec
                : 0
        };
    }
    ...
    public async Task<decimal?> ObterSelicAtual()
    {
            ...
            primeRateArray.GetArrayLength() > 0)
        {
            ...
            if (taxaElement.TryGetProperty("value", out JsonElement valueElement))
            {
                ...
                if (decimal.TryParse(valorString, NumberStyles.Any, CultureInfo.InvariantCulture, out decimal taxaDecimal))
                {
                    return taxaDecimal;
                }
            }
